...
## One-time manual rearm override

The v2.12 override exists only when white rearm and timed rearm are both enabled and the state is `WAITING_REARM`.

- Request timestamp is recorded after the menu closes.
- Samples are ignored for the override during the first `35 ms`.
- After that settle period, the override requires **2 consecutive `isArmingWhite()` samples**.
- A non-white sample resets the override white-frame counter to 0.
- If two white samples arrive, the engine arms immediately and bypasses the time delay only for that cycle.
- If `500 ms` elapse without satisfying the white requirement, the override is cleared without arming.
- If engine state/config becomes incompatible while the request is pending, the request is cleared.
